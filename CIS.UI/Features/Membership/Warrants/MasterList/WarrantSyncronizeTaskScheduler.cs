...
public class WarrantSyncronizeTaskScheduler : ITaskScheduler
{
    ...
    private IEnumerable<Store.Domain.Entities.Warrant> GetStoreData()
    {
        ...
        using (var transaction = session.BeginTransaction())
        {
            var sql = string.Format(@"
                ...
                where 
	                #Temp.Sequence between 1 and (select max(DataStoreId) from Membership.Suspects) and
	                #Temp.Sequence not in (select DataStoreId from Membership.Suspects where DataStoreId is not null)",
                App.Context.DataStore.FetchSize
            );

            request.FetchSize = App.Context.DataStore.FetchSize;
            ...
            request.Client = App.Context.GetClientInfo();
    ...
    private IEnumerable<Store.Domain.Entities.Warrant> GetLocalData()
    {
        ...
        using (var transaction = session.BeginTransaction())
        {
            var query = session.Query<Suspect>()
                ...
                .Take(App.Context.DataStore.FetchSize)
                .Fetch(x => x.Warrant)
                .Fetch(x => x.Aliases)
                .ToFuture();
    ...
    private void PushData()
    {
        ...
        while ((warrants = GetLocalData()).Count() > 0)
        {
            ...
            {
                FetchSize = App.Context.DataStore.FetchSize,
                Warrants = warrants.ToArray(),
                Client = App.Context.GetClientInfo()
            };
