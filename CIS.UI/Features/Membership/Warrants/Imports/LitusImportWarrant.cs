...
namespace CIS.UI.Features.Membership.Warrants.Imports
{
    public class LitusImportWarrant 
    {
        ...
        internal static LitusImportWarrant OfDataRow(DataRow row)
        {
            return new()
            {
                WarrantCode = row.Field<string>("CODE"),
                CaseNumber = row.Field<string>("CASE_NO"),
                Description = row.Field<string>("CRIME_TYPE_NAME"),
                BailAmount = row.Field<string>("BAIL_AMOUNT").ParseDecimal(),
                IssuedOn = row.Field<string>("ISSUED_ON").ParseDate(),
                IssuedBy = row.Field<string>("ISSUED_BY_NAME"),
