...
public class OfficerListController : ControllerBase<OfficerListViewModel>
{
    ...
    private OfficerViewModel New()
    {
        ...
        {
            var ranks = session.Query<MembershipD.Rank>().Cacheable().ToList();
    ...
    private OfficerViewModel Get(Guid id)
    {
        ...
        using (var transaction = session.BeginTransaction())
        {
            var rankQuery = session.Query<MembershipD.Rank>().Cacheable().ToList();

            var officerQuery = session.Query<MembershipD.Officer>()
                .Where(x => x.Id == id)
                .Fetch(x => x.Rank)
                .ToFutureValue();
    ...
    public virtual void Search()
    {
        ...
        var query = session.Query<MembershipD.Officer>();
    ...
    public virtual void Insert(OfficerViewModel value)
    {
        ...
        using (var transaction = session.BeginTransaction())
        {
            ...
            if (officer == null)
            {
                officer = new MembershipD.Officer();
    ...
    public virtual void Delete(OfficerListItemViewModel item)
    {
        try
        {
            ...
            using (var transaction = session.BeginTransaction())
            {
                var officer = session.Load<MembershipD.Officer>(item.Id);
