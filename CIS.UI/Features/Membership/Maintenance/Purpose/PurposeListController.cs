...
public class PurposeListController : ControllerBase<PurposeListViewModel>
{
    ...
    public virtual void Load()
    {
        ...
        var query = session.Query<MembershipD.Purpose>()
            .ToFuture();
    ...
    public virtual void Insert()
    {
        ...
        using (var transaction = session.BeginTransaction())
        {
            var exists = session.Query<MembershipD.Purpose>().Any(x => x.Name == this.ViewModel.NewItem);
            if (exists)
            {
            ...
            var entity = new MembershipD.Purpose() { Name = this.ViewModel.NewItem };
    ...
    public virtual void Delete(PurposeViewModel item)
    {
        ...
        using (var transaction = session.BeginTransaction())
        {
            var entity = session.Get<MembershipD.Purpose>(item.Id);
