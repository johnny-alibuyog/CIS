...
public class CameraController : ControllerBase<CameraViewModel>
{
    ...
    private void CaptureNewFrame(IFrameSource frameSource, Frame frame, double fps)
    {
        ...
        bitmap.ReduceSize(App.Context.Image.PictureResizeScaleFactor).Save(Path.Combine(App.Config.ApplicationDataLocation, "reducedPicture.bmp"));
#endif

        this.ViewModel.ImagePreview = bitmap.ReduceSize(App.Context.Image.PictureResizeScaleFactor).ToBitmapSource();
    }
    ...
    public virtual void Start()
    {
        ...
        {
            this.MessageBox.Warn("No camera selected or no camera available.", "Police Registration");
            return;
        }
