...
/// </summary>
void CameraMethods::DisplayCameraPropertiesDialog(int camIndex, IntPtr hwnd)
{
	if (camIndex >= Count)
		throw gcnew ArgumentException("Camera index is out of bounds: " + Count.ToString());
	...
	if (hwnd == IntPtr::Zero)
		throw gcnew ArgumentException("Null hwind pointer");
	...
	if (SUCCEEDED(hr))
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			OleCreatePropertyFrame(
				(HWND)hwnd.ToPointer(), // Parent window
				0, 0,                   // Reserved
